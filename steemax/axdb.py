...
class AXdb(DB):
    ...
    def first_time_setup(self):
        ...
        if not self.get_results("SELECT * FROM axlist WHERE 1;"):
            ...
            self.commit("CREATE TABLE IF NOT EXISTS axlist (ID int(10), Account1 varchar(50), "
                        + "Account2 varchar(50), Key1 varchar(100), Key2 varchar(100), "
                        + "Percentage varchar(5), Ratio varchar(5), Duration varchar(5), "
                        + "MemoID varchar(40), Status varchar(10), Time TIMESTAMP DEFAULT CURRENT_TIMESTAMP);")

            self.msg.message("Created axlist table in the steemax database.")

        if not self.get_results("SELECT * FROM axglobal WHERE 1;"):

            self.commit("CREATE TABLE IF NOT EXISTS axglobal (ID int(10), RewardBalance varchar(50), "
                        + "RecentClaims varchar(50), Base varchar(50), Time TIMESTAMP DEFAULT CURRENT_TIMESTAMP);")

            self.commit("INSERT INTO axglobal (ID, RewardBalance, RecentClaims, Base) VALUES ('1', '0', '0', '0');")

            self.msg.message("Created and initialized axglobal table in the steemax database.")

        if not self.get_results("SELECT * FROM axtrans WHERE 1;"):

            self.commit("CREATE TABLE IF NOT EXISTS axtrans (ID INT NOT NULL AUTO_INCREMENT PRIMARY KEY, "
                        + "TXID varchar(50), MemoFrom varchar(20), Amount varchar(20), MemoID varchar(40), "
                        + "Action varchar(20), TxTime TIMESTAMP NULL, "
                        + "DiscoveryTime TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP);")

            self.msg.message("Created axtrans table in the steemax database.")
    ...
    def add_trans(self, txid, memofrom, amt, memoid, action, txtime):
        ''' Adds a processed transaction to the axtrans database 
        table which is a history of all processed transactions
        '''
        self.commit("INSERT INTO axtrans (TXID, MemoFrom, Amount, MemoID, Action, TxTime) VALUES ('"
                    +txid+"', '"+memofrom+"', '"+amt+"', '"+memoid+"', '"+action+"', '"+txtime+"');")



    def check_reward_fund_renewal(self):
        ''' Checks the timestamp from the database to see if the stored 
    ...
    def save_reward_fund(self, rb, rc, base):
        ''' Saves the current Reward Balance, Recent Claims and the 
        price of STEEM in the database for quicker 
        retreival and less taxation on steem nodes.
        '''
        return self.commit("UPDATE axglobal SET RewardBalance = '"
                            +str(rb)+"', RecentClaims = '"+str(rc)+"', Base = '"
                            +str(base)+"' WHERE ID = '1';")



    def fetch_reward_fund(self):
        ''' Retrives the Reward Balance, Recent Claims and 
        the price of STEEM from the database instead of from
        the steem node
        '''
    ...
    def verify_memoid(self, acct0, memoid):
        ...
        '''
        if not self.get_results("SELECT Account1, Account2, Status FROM axlist WHERE MemoID = '" 
                                + memoid + "';"):
            self.msg.error_message("Could not find Memo ID.")
            return False
        ...
        if acct0!= acct1 and acct0 != acct2:
            self.msg.error_message("Verify Memo ID: Account names do not match.")
            return False
        else:
            return True
    ...
    def cancel (self, acct, memoid):
        ...
        '''
        return self.commit("DELETE FROM axlist WHERE (Account1 = '"
                            +acct+"' OR (Account2 = '"+acct+"')) AND (MemoID = '"
                            +memoid+"');")
    ...
    def update_invite(self, percent, ratio, duration, memoid, status):
        ...
        '''
        return self.commit("UPDATE axlist SET Percentage = '" + str(percent) 
                            + "', Ratio = '" + str(ratio) + "', Duration = '" 
                            + str(duration) + "', Status = '" + str(status) 
                            + "' WHERE MemoID = '" + str(memoid) + "';")
    ...
    def verify_account (self, acct, memoid):
        ...
        Then check to see if this is the invitee (Account2). 
        ...
        if self.get_results(sql):
            self.msg.message(acct + " is the inviter of " 
                        + str(len(self.dbresults)) + " exchange(s)")
        ...
        if self.get_results(sql):
            xmsg.message(acct + " is the invitee of " 
                        + str(len(self.dbresults)) + " exchange(s)")
    ...
    def verify_invitee (self, acct2, memoid):
        '''Check that account is truly an invitee 
        (Account2) and not inviter (Account1)
        '''
        if not self.get_results("SELECT * FROM axlist WHERE Account2 = '" 
                                + acct2 + "' AND (MemoID = '" + memoid + "');"):
    ...
    def accept_invite(self, acct2, memoid, key):
        ...
        '''
        return self.commit("UPDATE axlist SET Key2 = '" + key 
                            + "', Status = '0' WHERE Account2 = '" + acct2 
                            + "' AND (MemoID = '" + memoid + "');")
    ...
    def add_invite (self, acct1, acct2, key1, percent, ratio, duration):
        ...
        if self.get_results("SELECT * FROM axlist WHERE (Account1 = '" + acct1 
                            + "' OR (Account1 = '" + acct2 + "')) AND (Account2 = '" + 
            acct1 + "' OR (Account2 = '" + acct2 + "'));"):
            xmsg.message("An exchange has already been made between these accounts.")
            return False

        if self.commit("INSERT INTO axlist (Account1, Account2, Key1, Percentage, "
                        + "Ratio, Duration, MemoID, Status) VALUES ('" + acct1 
                        + "', '" + acct2 + "', '" + key1 + "', '" + percent + "', '" 
